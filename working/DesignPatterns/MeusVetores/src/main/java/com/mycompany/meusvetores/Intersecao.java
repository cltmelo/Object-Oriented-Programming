 ...
 * @author lucas
 */
public class Intersecao implements MeusVetoresBrigde {

    @Override
    public int[] executeOperation(MeusVetores conjuntos) {
        return (int[]) Arrays.stream(conjuntos.getConjunto1()).filter(value -> Arrays.stream(conjuntos.getConjunto2()).anyMatch(x -> x == value)).toArray();
    }
...
//        System.out.println("Conjunto 2: " + Arrays.toString(conjuntos.getConjunto2()));
        System.out.println("Resultado Interseção: " + Arrays.toString(executeOperation(conjuntos)));
        System.out.println();
    }
